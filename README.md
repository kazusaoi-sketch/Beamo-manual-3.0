...
## 새 카테고리 (새로운 버전 및 기능 출시 시) 추가하는 법
...
> 새 카테고리 `[영문-slug]` 추가해줘. 화면 표시 이름은 한국어 '[한글명]', 영어 '[English name]', 일본어 '[日本語名]'이야.

새 버전을 추가할 때:

> 새 버전 `[버전번호]` 추가해줘. 화면 표시 이름은 한국어 '[표시명]', 영어 '[Display name]', 일본어 '[表示名]'이고, [사이트 안에서 정식 브라우징 대상 / 2.0처럼 외부 링크 안내] 중 [선택]으로 처리해줘.

이 한 줄이면 위 세 파일(`.github/workflows/translate-new-doc.yml`, `app.js`, `i18n-ui.js`)을 한 번에 찾아서 수정해줍니다.
